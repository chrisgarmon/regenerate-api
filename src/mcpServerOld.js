...
// OPTION A: SSE Transport (simple, works with ChatGPT)
const transport = new SSEServerTransport("/sse", new express.Response());
...
app.post("/messages", async (req, res) => {
  ...
  try {
    const response = await server.callTool(messages[messages.length - 1]);
    res.json(response);
  } catch (error) {
